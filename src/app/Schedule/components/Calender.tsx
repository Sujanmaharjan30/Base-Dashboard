...
const Calender = () => {
  const [currentDate, setCurrentDate] = useState(new Date(2021, 11, 2)); // December 2021
  const [selectedDate, setSelectedDate] = useState(3); // Selected date 3
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    calendarDays.push({
      day,
      isCurrentMonth: true,
      isSelected: day === selectedDate && month === 11 && year === 2021
    });
  }
  ...
  return (
    <div className='mt-5 mx-7.5 border border-[#F7F7F8] rounded-[7px] p-4 w-[206px]' style={{ fontFamily: "Nunito" }}>
      {/* Header */}
      ...
      <div className='grid grid-cols-7 gap-1'>
        {calendarDays.slice(0, 35).map((item, index) => (
          <button
            ...
            className={`text-center text-xs py-2 flex items-center justify-center w-6 h-6 rounded ${
              item.isSelected
